...
class Nina5Processor(BaseProcessor):
    ...
    def __init__(self,
        ...
        self.pemgs, self.pimus, self.plbls, self.preps = None, None, None, None
    ...
    def _load_file(self, path: str, ex: str) -> tuple[Any, list[Any] | Any, int | Any, Any]:
        ...
        emgs = res['emg'].copy()
        imus = res['acc'].copy() if self.use_imu else []
    ...
    def _load_by_exercises(self, idx: int, ex: str) -> tuple[list[Any], list[Any], list[Any], list[Any]]:
        """
        ...
        for sub in tqdm(range(1, self.NUM_SUBJECTS + 1), file=sys.stdout):
            ...
            emg, imu, lbl, rep = self._load_file(str(path), ex)
            emgs += [emg]
            imus += [imu] if self.use_imu else imu
            lbls += [lbl]
            reps += [rep]
        return emgs, imus, lbls, reps
    ...
    def load_data(self) -> None:
        ...
        for i, ex in enumerate(self.BASE_LABEL_IDS.keys()):
            ...
            emgs, imus, lbls, reps = self._load_by_exercises((i + 1), ex)
            self.emgs += emgs
            self.imus += imus
            self.lbls += lbls
            self.reps += reps
    ...
    def _process_data_v1(self) -> None:
        ...
        if self.use_butter:
            ...
            self.pemgs = [butter_band(emg, lcut=5., hcut=99., fs=200, order=4) for emg in self.pemgs]
        ...
        if self.use_imu:
            self.pimus = [window_rolling(imu, self.step_size, self.window_size) for imu in self.imus]
            self.pimus = np.moveaxis(np.concatenate(self.pimus, axis=0), 2, 1)
            self.pimus = self.pimus[no_leaks, :, :]
        else:
            self.pimus = self.imus
        # release memory
        del no_leaks
        gc.collect()
        ...
        if self.use_imu:
            self.pimus = self.pimus.astype(np.float16)
        ...
        if not self.use_rest_label:
            ...
            if self.use_imu:
                self.pimus = self.pimus[np.where(self.plbls != 0)[0]]
    ...
    def _process_data_v2(self) -> None:
        ...
        if self.use_butter:
            ...
            print('Butterworth filtering...')
            self.pemgs = process_butter_band(self.pemgs, lcut=5., hcut=99., fs=200., order=4)
    ...
    def split_data(self, split: str) -> dict[str, Any]:
        ...
        if split == 'train':
            ...
            data = dict(emg=self.pemgs[idxs].copy(),
                        imu=self.pimus[idxs].copy() if self.use_imu else self.pimus,
